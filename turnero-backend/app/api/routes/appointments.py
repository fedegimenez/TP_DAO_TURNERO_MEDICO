...
def create_turno(payload: AppointmentCreate, db: Session = Depends(get_db),
...
def update_turno(tid: int, payload: AppointmentUpdate, db: Session = Depends(get_db),
                 _=Depends(require_role("admin", "medico"))):
    try:
...
def cancelar_turno(tid: int, db: Session = Depends(get_db),
                   _=Depends(require_role("admin", "medico"))):
    try:
...
def atender_turno(tid: int, receta_url: str | None = None, db: Session = Depends(get_db),
                  _=Depends(require_role("admin", "medico"))):
    try:
...
def registrar_consulta(tid: int, payload: ConsultationCreate, db: Session = Depends(get_db),
                       _=Depends(require_role("admin", "medico"))):
    try:
...
def programar_recordatorio(tid: int, payload: ReminderCreate, db: Session = Depends(get_db),
                           _=Depends(require_role("admin", "medico"))):
    try:
